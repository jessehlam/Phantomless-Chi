...
mu_figure.DefaultAxesLineStyleOrder='o|o|-';
mu_figure.DefaultLineMarkerSize= 1;
...
for a=1:nDiodes,
	...
	plot(final.freq, final.phase(:,a),'b*'); %[haxes, hline1, hline2] = fit.freq, (fit.rawphi(:,a) - fit.phi(:,a)), 'plot'
	haxes = gca;
	axes_pos = get(haxes, 'Position');
	set(haxes, 'Position', [axes_pos(1)+0.05 axes_pos(2)-.01 axes_pos(3) axes_pos(4)]);
	hold on;
	plot(final.freq, fit.phi(:,a), 'r*','markersize',1);
	if a == 1, 
		axes('Position', axes_pos,'Visible','off');
